...
void do_exam1_pointer();
void do_exam2_pointer();
void do_exam3_pointer();
void do_exam4_pointer();
void do_exam5_pointer();
void do_exam6_pointer();
void do_exam7_pointer();
void do_exam8_pointer();
...
int main()
{
    ...
    switch(choice) 
	{
        ...
        case 21: 
            ...
            break;            
        default:
            printf("잘못된 입력입니다. 1에서 21 사이의 번호를 입력해주세요.\n");
            break;
    }
    return 0;
}
...
void do_exam1_pointer()
{
	...
	printf("정수 변수 i: 값 = %d, 주소 = %p\n", i, &i);
	printf("문자 변수 c: 값 = %c, 주소 = %p\n", c, &c);
	printf("실수 변수 f: 값 = %.1f, 주소 = %p\n", f, &f);
} 
...
void do_exam4_pointer()
{
	...
	printf("정수 i: 직접참조 = %d, 간접참조(*p) = %d\n", i, *p);
	printf("문자 c: 직접참조 = %c, 간접참조(*pc) = %c\n", c, *pc);
	printf("실수 f: 직접참조 = %.1f, 간접참조(*pf) = %.1f\n", f, *pf);
}  
...
void do_exam2_array()
{
	...
	float costs[12];
	costs[0] = 0.2f; costs[1] = 1.3f; costs[2] = 5.6f;
	...
	while(1) 
    {
		...
		switch(choice) 
        {
            case 1:
                ...
                scanf("%d", &amount);
                balance += amount;
                printf("%d원이 입금되었습니다. 현재 잔액: %d원\n", amount, balance);
                break;

            case 2:
                printf("\n[출금] 출금 로직이 실행됩니다...\n");
                printf("[출금] 출금할 금액을 입력해주세요. (출금진행중...): ");
                scanf("%d", &amount);
                if (balance >= amount) 
                {
                    balance -= amount;
